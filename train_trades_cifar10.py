...
from trades import trades_loss, diff_loss

os.environ['CUDA_VISIBLE_DEVICES'] = "1"
# wandb.init(project="trade-loss_monitor", entity="lovettxh", name="cifar-trade-temp")
...
def train(args, model, device, train_loader, optimizer, epoch, adv_flag):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        optimizer.zero_grad()
        loss = diff_loss(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
...
def main():
    ...
    if args.continue_train != 0:
        ...
        for epoch in range(start+1, length+start+1):
            ...
            avg = train(args, model, device, train_loader, optimizer, epoch)
        ...
        #     if epoch >= 76:
        #         torch.save(model.state_dict(),
        #                 os.path.join(model_dir, 'wideres-nat-epoch{}.pt'.format(epoch)))
        #         torch.save(optimizer.state_dict(),
        #                 os.path.join(model_dir, 'opt-wideres-nat-checkpoint_epoch{}.tar'.format(epoch)))
        del model
        del optimizer
        model_ = WideResNet().to(device)
        #model = torch.nn.parallel.DistributedDataParallel(model, find_unused_parameters=True)
        optimizer_ = optim.SGD(model_.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        for epoch in range(1, args.epochs + 1):
            # wandb.log({"epoch":epoch})
            # adjust learning rate for SGD
            adjust_learning_rate(optimizer_, epoch)
            # adversarial training
            train(args, model_, device, train_loader, optimizer_, epoch, True)
            # evaluation on natural examples
            print('================================================================', flush=True, file=f)
            eval_train(model_, device, train_loader, epoch)
            eval_test(model_, device, test_loader, epoch)
            print('================================================================', flush=True, file=f)
            # save checkpoint
            if epoch >= 76:
                torch.save(model_.state_dict(),
                        os.path.join(model_dir, 'wideres-adv-epoch{}.pt'.format(epoch)))
                torch.save(optimizer_.state_dict(),
                        os.path.join(model_dir, 'opt-wideres-adv-checkpoint_epoch{}.tar'.format(epoch)))
